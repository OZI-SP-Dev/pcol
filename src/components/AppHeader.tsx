import {
  ...
  export const AppHeader = () => {
    ...
    const title =
      "PCOL" +
      ((import.meta.env.MODE === "testing" || import.meta.env.DEV) && " TEST");
  
    return (
      <>
        <div role="heading" aria-level={1} className={classes.navHeader}>
          <Link className={classes.navLink} to="/">
            <Text className={classes.navHeaderSiteName}>{title} Home</Text>
          </Link>
          {match?.params?.program && 
            <Link className={classes.navLink} to={"/p/" + match?.params?.program}>
              <Text className={classes.navHeaderSiteName}>{match?.params?.program} Home</Text>
            </Link>
          }
          ...
          <Link to="/Help" className={classes.navHelp} >
            Help
          </Link>
        </div>
